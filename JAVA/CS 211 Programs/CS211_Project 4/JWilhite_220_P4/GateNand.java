/*
   *********************************************************
    GateNand class for Project 4 of CS211-220 (Spring 2017)
...
{
	//Constructors:
	public GateNand(List<Wire> ins, Wire output) //Constructor Type 0: input and output provided. (COMPLETE)
	{
		super("NAND", ins, output); //calls parent constructor with name = "NAND" , ins = ins, and output = output.
	}
	...
	@Override public boolean propagate() //This method applies the Gate-specific logic on the input signals, returns true if the signal propagates properly, false otherwise: (COMPLETE)
	{
		...
		if(has_lows == true) //if any of the outputs are LO:
		{
			(this.getOutput()).setSignal(Signal.HI); //set the output signal to HI.
			return true; //return true.
		}
		
		if(has_highs == true && has_lows == false && has_unknowns == true) //if HI's and X's were detected in the inputs, and no LO's were detected:
		{
			(this.getOutput()).setSignal(Signal.X); //set the output signal to X.
			return true; //return true
		}
		
		if(has_highs == true && has_lows == false && has_unknowns == false) //if there were only high's detected:
		{
