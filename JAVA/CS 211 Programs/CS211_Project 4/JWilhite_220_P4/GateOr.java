/*
   *******************************************************
    GateOr class for Project 4 of CS211-220 (Spring 2017)
...
{
	//Constructors:
	public GateOr(List<Wire> ins, Wire output) //Constructor Type 0: input and output provided. (COMPLETE)
	{
		super("OR", ins, output); //calls parent constructor with name = "OR" , ins = ins, and output = output.
	}
	...
	@Override public boolean propagate() //This method applies the Gate-specific logic on the input signals, returns true if the signal propagates properly, false otherwise: (COMPLETE)
	{
		...
		if(has_highs == false && has_lows == false && has_unknowns == true) //if only X's were detected:
		{
		...
		if(has_highs == false && has_lows == true && has_unknowns == false) //if only LO's were detected:
		{
